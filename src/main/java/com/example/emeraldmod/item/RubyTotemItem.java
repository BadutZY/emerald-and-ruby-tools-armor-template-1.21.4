...
/**
 * Ruby Totem of Undying - Can prevent death 5 times
 */
public class RubyTotemItem extends Item {
    private static final int MAX_USES = 5;
    private static final String NBT_USES_KEY = "RemainingUses";

    public RubyTotemItem(Settings settings) {
        super(settings);
    }
    ...
    public static ItemStack createTotem() {
        ItemStack stack = new ItemStack(ModItems.RUBY_TOTEM);
    ...
    /**
     * Check if this stack is a Ruby Totem
     */
    public static boolean isRubyTotem(ItemStack stack) {
        return stack.getItem() instanceof RubyTotemItem;
    }
    ...
    public void appendTooltip(ItemStack stack, TooltipContext context, List<Text> tooltip, net.minecraft.item.tooltip.TooltipType type) {
        ...
        tooltip.add(Text.literal("Ruby Totem of Undying")
                .formatted(Formatting.RED, Formatting.BOLD));
        tooltip.add(Text.literal("Uses Remaining: " + uses + "/" + MAX_USES)
                .formatted(Formatting.LIGHT_PURPLE));
        tooltip.add(Text.literal(""));
        tooltip.add(Text.literal("Prevents death up to 5 times")
    ...
    /**
     * Durability bar color (red/ruby)
     */
    @Override
    public int getItemBarColor(ItemStack stack) {
        int uses = getRemainingUses(stack);
        if (uses == 5) return 0xFF68F5; // Ruby red
        if (uses >= 4) return 0xFF1744; // Bright red
        if (uses >= 3) return 0xFF6B6B; // Light red
        if (uses >= 2) return 0xFFA500; // Orange
        return 0xFF0000; // Pure red (warning)
    }
}